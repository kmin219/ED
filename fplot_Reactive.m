...
xmarkers1 = Reactive_EventA(:,1);   % Where the events are detected in time difference domain
xmarkers2 = (xmarkers1-1).*Window_Shift+Window_Size;   % Where the events are detected in time domain
ymarkers1 = Reactive_EventA(:,2);   % Power consumption

figure();
n = [1:length(Reactive)];
ax(1) = subplot(211);plot(n, Reactive(n), 'r'); 
ax(2) = subplot(212);plot(n, Reactive(n), xmarkers2,ymarkers1,'r*');
