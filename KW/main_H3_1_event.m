...
for i=1:length(ts_start)
    ...
    text(double(start_idx(i)),100,['ON (Tagging Info)'] );
    text(double(end_idx(i)),100,['OFF (Tagging Info)'] );
end
hold off;
ax(2) = subplot(212);plot(n, featP_L1(n));grid;hold on;
for i = [1:max(Windows_All_L1(:,3))]
    x_temp = min(find(Windows_All_L1(:,3)==i));
    x_event = (Windows_All_L1(x_temp,1)-1).*Window_Shift+Window_Size*2+Window_Dist;
    if (floor(i/2) == i/2)
        line([x_event x_event], [0,100],'Color','g', 'LineWidth',2);
        text(x_event,100,['OFF (Corrected)'] );
    else
        line([x_event x_event], [0,100],'Color','r', 'LineWidth',2);
        text(x_event,100,['ON (Corrected)'] );
    end
end
hold off;
...
for i = [1:size(A,1)]
    on_start_idx = fgetindex(Windows_All_L2,A(i,1),A(i,2),A(i,3));
    on_end_idx = fgetindex(Windows_All_L2,A(i,4),A(i,5),A(i,6));
    window_P{i}{1} = featP_scanA_L2(on_start_idx:on_end_idx);  % on windows
    
    off_start_idx = fgetindex(Windows_All_L2,A(i,7),A(i,8),A(i,9));
    off_end_idx = fgetindex(Windows_All_L2,A(i,10),A(i,11),A(i,12));
