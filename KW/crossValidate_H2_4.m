...
idx = cell2mat(idx_H2_4(:,1));


load window_P_H2_4
load window_Q_H2_4
window_P = window_P_H2_4;
window_Q = window_Q_H2_4;
...
%         MetricQ(find(MetricQ>sqrt(sum(abs(window_Q{i}{1}))))) = sqrt(sum(window_Q{i}{1}.^2));
        ...
        title(sprintf('P Monitor: "%s"', char(idx_H2_4(i,2))))
        ax(2) = subplot(412);plot(n,MetricQ);
        title(sprintf('Q Monitor: "%s"', char(idx_H2_4(i,2))))
        subplot(413);plot(window_P{i}{1});
        title(sprintf('Appliance P Window: "%s"', char(idx_H2_4(i,2))))
        subplot(414);plot(window_Q{i}{1});
        title(sprintf('Appliance Q Window: "%s"', char(idx_H2_4(i,2))))
        linkaxes(ax, 'x');
        pause;
    end
end
