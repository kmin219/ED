function [figure_handle] = Plot_Single(tag, ProcessedData, hasLabels, varargin)
...
if(tag == 1)
    ...
    if(hasLabels == true && isfield(ProcessedData,'TaggingInfo') )
        ...
        for i=1:size(ProcessedData.TaggingInfo,1)
            line([ProcessedData.TaggingInfo{i,3},ProcessedData.TaggingInfo{i,3}],[0,100],'Color','g','LineWidth',2);
            %We add a little offset for display purposes to end marker since event could be +- 30
            %seconds of the timestamp.
            offset = 0;
            line([ProcessedData.TaggingInfo{i,4} + offset,ProcessedData.TaggingInfo{i,4} + offset],[0,100],'Color','r', 'LineWidth',2);    
            text(double(ProcessedData.TaggingInfo{i,3}),100,['ON-' ProcessedData.TaggingInfo{i, 2}] );
            text(double(ProcessedData.TaggingInfo{i,4}),100,['OFF'] );
        end
    end
...
%     Plot Imaginary/Reactive power (VAR)
    plot(ProcessedData.L1_TimeTicks, ProcessedData.L1_Imag); grid; hold on;
...
elseif(tag == 3)
    % Plot Power Factor
    plot(ProcessedData.L1_TimeTicks, ProcessedData.L1_Pf); grid; hold on;
