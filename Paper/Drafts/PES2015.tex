...
\begin{figure}[!h]	
	\centering
		\subfloat[Training]{\includegraphics[width=3.7in,height = 1.05in]{fig/exTrain.eps}
			\label{fig:event1ml}}
		\hfil
		\subfloat[Test]{\includegraphics[width=3.7in,height = 1.05in]{fig/exTestH2_1.eps}
			\label{fig:event2ml}}
...
In this Section, the signature windows trained in Section XX are cross-validated using the training set.  The RMSE values shown in (\ref{eq:RMSE-P})-(\ref{eq:RMSE-Q}) are used as metrics for the evaluation.  
\begin{align}
\label{eq:RMSE-P}
...
\begin{algorithmic}
	...
	\IF{$RMSE_P(n_0)<\theta_P$ and $RMSE_Q(n_0)<\theta_Q$ } 
...
need pseudo code for determining if appliance is separable or not, eg,
apply signature window w(n)i,j
determine if threshhold (theta) exists for which D(n)<1.3*theta is able to detect 100 \% of appliances i in the training set and none of the other  N-1 appliances in the training set (that's why 1.3*theta)
need to determine P and Q thresholds simultaneously

\begin{equation}
RMSE_{P}(n) = \sqrt{\frac{1}{N}\sum_{k=0}^{N-1}\left(\Delta P_{avg}(n-k)-w_{i,j}(k)\right)^2}
\end{equation}
...
\begin{table}[h!]
	...
	\begin{tabular}{c||c||c||c}
		\hline 
		\textbf{Home No.} & \textbf{Validated} &\textbf{Discarded} &\textbf{\% Validated}\tabularnewline
		\hline 
		\hline 
		H1 & 38 & 6 & 4\tabularnewline
		\hline 
