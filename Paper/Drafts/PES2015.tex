...
%%		\label{fig:event1}}
%	\subfloat[]{\includegraphics[width=3.5in, height = 1in]{fig/fan.eps}
%		\label{fig:event2}}
%	\hfill
%%	\subfloat[]{\includegraphics[width=1.7in]{fig/computer1.eps}
%%		\label{fig:event1}}
%	\subfloat[]{\includegraphics[width=3.5in,height = 1in]{fig/TV.eps}
%		\label{fig:event2}}
%	\hfill
%	\subfloat[]{\includegraphics[width=3.5in,height = 1in]{fig/dryer.eps}
%		\label{fig:event1}}
...
When the signature window is scanning its target, both the $\text{RMSE}_{\text{P}}$ and $\text{RMSE}_{\text{Q}}$ values fall very low.  On the other hand, if the window is scanning on a different time stamp, each $\text{RMSE}_{\text{P}}$ and $\text{RMSE}_{\text{Q}}$ value can rise, fall, or stay the same.  It is possible that both the RMSE values drop when the window is detecting different appliance.  This happens when the signature windows of the two appliances have a very similar shape and amplitude.  To prevent this scenario, RMSE thresholds ($\theta_P$ and $\theta_Q$) is set to separate these appliance from being detected.  If a RMSE threshold could not be set, the window is discarded.  Figure \ref{fig:bread} shows the detection result after applying the turn-on window of a breadmaker to the training set.  The window successfully detects the breadmaker turning-on with all the other events being not detected. 
